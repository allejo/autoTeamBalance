...
public:
    ...
    double timeFirstUneven; // The server timestamp teams were first marked as uneven
...
void teamSwitch::Event (bz_EventData* eventData)
{
    switch (eventData->eventType)
    {
        ...
        case bz_ePlayerPartEvent:
        {
            ...
            players.erase(partData->playerID);
        }
        break;
...
{
    if ((command == "switch" || command == "balance") && !bz_hasPerm(playerID, "switch"))
    {
        bz_sendTextMessage(BZ_SERVER, playerID, "You do not have permission to use this command.");
        return true;
    }

...
bz_APIIntList* teamSwitch::getStrongestTeamPlayers(bz_eTeamType team, int numberOfPlayers)
{
    ...
    for (unsigned int i = 0; i < playerlist->size(); i++)
    {
        ...
        // increase by 1 to avaoid a division by zero error
        wins++;
        losses++;
...
bool teamSwitch::playerCountLow ()
{
    ...
    // Allow zero team count so if all players in a team leave, it will auto balance other team if difference > 1
    return (bz_getTeamCount(TEAM_ONE) < 4 && bz_getTeamCount(TEAM_TWO) < 4 && difference < 2);
}
